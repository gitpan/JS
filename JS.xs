...
MODULE = JS     PACKAGE = JS::RawRT	PREFIX = jsr_
	...
	if(SvOK(sv) && SvROK(sv) && (sv = SvRV(sv)) && SvMAGICAL(sv)) {
		...
		if(mg->mg_type == PERL_MAGIC_jsvis &&
		  ...
		  jsvis->pcx == pcx
			...
			"__new",
			sv_2mortal(newSVpv("JS::Visitor", 0)),	// package
...
MODULE = JS    PACKAGE = JS::Controller
...
BOOT:
    PJS_Context_SV = gv_fetchpv("JS::Context::CURRENT", GV_ADDMULTI, SVt_IV);
    PJS_This = gv_fetchpv("JS::This", GV_ADDMULTI, SVt_PV);
#if PJS_UTF8_NATIVE
