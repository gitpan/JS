...
use strict;
use warnings;
our @ISA = qw(JS::Object);
use Carp();

sub JS::Context::get_controller {
    my $ctx = shift;
...
sub toSource {
    ...
    my $val;
    if($t && $t->isa('JS::Object') || 
       Scalar::Util::blessed($v) && $v->isa('JS::Object') && ($t=$v)
    ) {
	$val = $t->toSource();
    } elsif($rt) {
	for($rt) {
	    /^HASH$/ || /^JS::PerlHash$/ and do {
		$val = JS::PerlHash::toSource($v); last
	    };
	    /^ARRAY$/ || /^JS::PerlArray$/ and do {
		$val = JS::PerlArray::toSource($v); last
	    };
	    /^CODE$/ and do { $val = JS::PerlSub::toSource($v); last };
	    $val = $rt;
	}
...
package JS::PerlScalar;

my $scalar;
our $prototype = \$scalar;

sub toString {
    my $this = shift || $JS::This;
    "${$this}";
}
...
sub toSource {
    my $aref = shift || $JS::This;
    $aref = $$aref if ref($aref) eq __PACKAGE__;
    "new PerlArray(" .  join(',', map JS::Any::toSource($_), @$aref) .  ")";
}
...
sub AUTOLOAD {
    ...
    my $metref = JS::Context::current->get_global
...
sub toSource {
    my $href = shift || $JS::This;
    my $cont = '';
    $href = $$href if ref($href) eq __PACKAGE__;
    while(my($k, $v) = each %{$href}) {
	$cont .= "'$k'," . JS::Any::toSource($v) . ',';
    }
    chop $cont if $cont;
    "new PerlHash($cont)";
}
...
JS::Controller - Control which Perl namespaces can be used from javascript. 
...
See L<JS::Stash> for details on C<Stash> intances.
...
javascript, if not in there already.  Returns the L<JS::Stash>
...
(the L<JS::Stash> object) if $package_name is in the list, otherwise
returns a FALSE value.

Normal operation is to automatically add namespaces as needed. Packages are added when
a perl object enters javacript or you use L<JS::Context/bind_class> and the
...
associated with the I<$package>. See L<JS::Stash> for all the
implications.
...
An advantage of this method over using L<JS::Context/bind_function> is
