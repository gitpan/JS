...
JS::PerlObject - Encapsulates generic perl objects in javascript

=head1 DESCRIPTION

When a perl object enters javascript land JS's engine will select the
kind of wrapper needed for it.  If the object belongs to a javascript class
created by L<JS::Context/bind_class> it becomes an instance of that
...
L<JS/"From perl to javascript">. Unless you have override on javascript that property, of course.
...
the JS module implements PerlObject's magic. Here you will see an
...
I<blessed>.  See L<JS::Stash> for the details.
