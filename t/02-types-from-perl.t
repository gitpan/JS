...
$cx1->bind_all(
	...
	my $ref = shift;
	isa_ok($ref, 'HASH');
	isa_ok(JS::Context::current->jsvisitor($ref), 'JS::Visitor',
	    "Now a visitor");
    }, 
);
...

{
    my $h = {};
    my $h2 = { a => 1, 'b' => 2 };
    ok(!JS::jsvisitor($h2), "Not a visitor yet");
    $cx1->call(test_hash => $h, $h2);
    ok(!JS::jsvisitor($h2), "Was a visitor");
    $h = undef;
    $h2 = undef;
}
...
ok(JS::jsvisitor(\&ok), "A visitor");
}
ok(!JS::jsvisitor(\&ok), "But not now");
ok(1, "All done");
