...
sub JS::PerlHash::new { bless {}, 'JS::PerlHash' }
sub JS::PerlHash::get_ref { $_[0]; }
$JS::PerlHash::construct_blessed = 1;

my $rt = JS::Runtime->new();
my $cx = $rt->create_context();
$cx->bind_function(ok => \&ok);

ok($JS::PerlHash::construct_blessed, "In legacy mode");
{
    my $hash = JS::PerlHash->new();
    ok(defined $hash, "Defined");
    isa_ok($hash, "JS::PerlHash");
...
/);
    ...
    isa_ok($hash, "JS::PerlHash", "Legacy wrapper");
...
$JS::PerlHash::construct_blessed = undef;
{
    my $hash = $cx->eval(q/
